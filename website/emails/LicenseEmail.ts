import { createElement as h } from "react";
import { Body, Container, Head, Heading, Hr, Html, Img, Preview, Section, Text } from "@react-email/components";

// Same small (40px) white-background render of the app icon as src/adapters/notify/content.ts's
// alert email, base64-inlined rather than linked. A remote image URL depends on guessing/deriving
// the right host correctly and on the recipient's mail client choosing to load remote images at
// all (most block them by default until the user clicks "show images") — inlining sidesteps both.
const LOGO_DATA_URI =
  "data:image/png;base64,";

/** The license-delivery email a customer receives from the /admin portal (website/api/admin/licenses.ts)
 * and from the daily expiry-reminder cron. Rendered to a static HTML string via @react-email/render at
 * send time — email clients don't run JS or read stylesheets reliably, so every style here is inline,
 * and layout uses plain block elements rather than flex/grid (Outlook's Word-based renderer ignores both).
 *
 * Written with React.createElement instead of JSX so this stays a plain .ts file: a .tsx sibling
 * module (not the api/ entry file itself) failed to bundle on Vercel's serverless function tracer
 * at runtime ("Cannot find module '.../emails/LicenseEmail.js'") even though the equivalent .ts
 * files elsewhere in this same import graph (lib/adminAuth.ts, lib/mail.ts) resolve fine — a
 * .tsx-specific gap in how the function's dependency trace maps a ".js" import specifier back to
 * its source file. Confirmed live in production logs before this rewrite. */
export function LicenseEmail(props: {
  customer: string;
  planLabel: string;
  deviceLimit: number;
  expiryText: string;
  licenseFileName: string;
  licenseFile: string;
}) {
  const rows: [string, string][] = [
    ["Plan", props.planLabel],
    ["Devices", String(props.deviceLimit)],
    ["Expires", props.expiryText],
  ];

  return h(
    Html,
    null,
    h(Head, null),
    h(Preview, null, `Your Argus license for ${props.customer} — ${props.planLabel}, ${props.deviceLimit} devices`),
    h(
      Body,
      { style: { backgroundColor: "#f4f4f7", margin: 0, padding: "32px 16px", fontFamily: "Segoe UI, Helvetica, Arial, sans-serif" } },
      h(
        Container,
        { style: { maxWidth: 520, margin: "0 auto", backgroundColor: "#ffffff", borderRadius: 16, overflow: "hidden", border: "1px solid #e5e5ea" } },
        h(
          Section,
          { style: { padding: "28px 32px 20px", borderBottom: "1px solid #efeff4" } },
          h(Img, {
            src: LOGO_DATA_URI,
            width: 32,
            height: 32,
            alt: "Argus",
            style: { display: "inline-block", verticalAlign: "middle", borderRadius: 8 },
          }),
          h(
            "span",
            { style: { display: "inline-block", verticalAlign: "middle", marginLeft: 10, fontSize: 17, fontWeight: 700, color: "#18181b" } },
            "Argus"
          )
        ),
        h(
          Section,
          { style: { padding: "28px 32px 8px" } },
          h(Heading, { style: { fontSize: 20, fontWeight: 700, color: "#18181b", margin: "0 0 16px" } }, "Your Argus license is ready"),
          h(
            Text,
            { style: { fontSize: 14, lineHeight: "22px", color: "#3f3f46", margin: "0 0 20px" } },
            `Hi ${props.customer}, your license is attached (${props.licenseFileName}) and included below.`
          ),
          h(
            "table",
            { role: "presentation", width: "100%", cellPadding: 0, cellSpacing: 0, style: { borderCollapse: "collapse", marginBottom: 20 } },
            h(
              "tbody",
              null,
              ...rows.map(([label, value]) =>
                h(
                  "tr",
                  { key: label },
                  h("td", { style: { padding: "8px 0", fontSize: 13, color: "#71717a", width: 110, borderBottom: "1px solid #f4f4f7" } }, label),
                  h(
                    "td",
                    { style: { padding: "8px 0", fontSize: 13, fontWeight: 600, color: "#18181b", borderBottom: "1px solid #f4f4f7" } },
                    value
                  )
                )
              )
            )
          ),
          h(
            Text,
            { style: { fontSize: 14, lineHeight: "22px", color: "#3f3f46", margin: "0 0 12px" } },
            h("strong", null, "To apply it:"),
            ' open Argus, sign in as an admin, go to Settings → License, paste the text below into "Apply a license", and click Apply.'
          ),
          h(
            "div",
            {
              style: {
                backgroundColor: "#18181b",
                color: "#e4e4e7",
                padding: 14,
                borderRadius: 10,
                fontFamily: "Consolas, Menlo, monospace",
                fontSize: 11,
                lineHeight: "16px",
                wordBreak: "break-all",
                whiteSpace: "pre-wrap",
                margin: "0 0 20px",
              },
            },
            props.licenseFile
          ),
          h(Hr, { style: { borderColor: "#efeff4", margin: "0 0 16px" } }),
          h(Text, { style: { fontSize: 13, lineHeight: "20px", color: "#71717a", margin: 0 } }, "Questions? Just reply to this email.")
        )
      ),
      h(
        Text,
        { style: { fontSize: 11, color: "#a1a1aa", textAlign: "center", margin: "20px 0 0" } },
        "Sent by Argus — network monitoring that never blinks."
      )
    )
  );
}
