...
; AppVerName explicitly set to just AppName: when left unset, Inno Setup's own *default* for
; AppVerName is "{AppName} version {AppVersion}" (confirmed by direct compile — a wrongly-recalled
; assumption in an earlier version of this comment claimed the fallback was AppName alone, with no
; version). AppVerName drives both the wizard's title bar caption and the Add/Remove Programs
; display name, so leaving it unset put "version X.Y.Z" in both places. AppVersion is still tracked
; separately (visible in the "Version" column of Programs and Features either way).
...
WizardStyle=modern
; The corner graphic Inno Setup shows on every wizard page except Welcome/Finish. Without this it
; silently falls back to Inno's own generic stock image, which isn't Argus branding at all — easy
; to mistake for "the real icon looks bad" when it's actually just not our icon. Generated by
; scripts/generate-icon.py at 192px (well above the 16-24px sizes where the pulse-line detail has
; to be sacrificed for legibility), so it stays crisp here.
